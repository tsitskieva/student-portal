...
class WeekManager(
    ...
    private val monthYearFormat =
        SimpleDateFormat("LLLL yyyy", Locale("ru")) // Формат для отображения месяца и года
    private var selectedDayOfWeek: Int = calendar.get(Calendar.DAY_OF_WEEK) // Выбранный день недели

    init {
        calendar.firstDayOfWeek =
            Calendar.MONDAY // Устанавливаем понедельник как первый день недели
    ...
    private fun updateWeek() {
        ...
        for (i in 0 until 7) {
            ...
            if (currentDayOfWeek == selectedDayOfWeek) {
                ...
                weekDays[i].background =
                    ContextCompat.getDrawable(weekDays[i].context, R.drawable.circle_background)
            }
        ...
        monthYearTextView.text =
            monthYearText.replaceFirstChar { it.uppercase() } // Делаем первую букву заглавной
    ...
    fun setSelectedDayOfWeek(dayOfWeek: Int) {
        ...
        Log.d("com.example.studentportal.ui.utils.WeekManager", "Selected day of week: $dayOfWeek")
    }
    ...
    fun getCurrentDayIndex(): Int {
        ...
        Log.d(
            "com.example.studentportal.ui.utils.WeekManager",
            "Current date: ${calendar.time}, Day of week: $dayOfWeek"
        )
